...
export const CreateStream = () => {
  ...
  function CreateButton({ isLoading, children, ...props }) {
    return (
      <Button variant="success" className="button my-4 text-sky-400/100 p-4 bg-amber-300 w-64 mx-24 " {...props}>
        {isButtonLoading ? <CircularProgress size={24} color="primary" /> : children}
      </Button>
    );
  }
  ...
  return (
    <div className="my-20">
      <h2 className="text-sky-400/100 p-4 bg-stone-100 text-center">Create a Stream to Pay to your Employees</h2>
      {currentAccount === "" ? (
        <button id="connectWallet" className="button rounded-full p-4 text-center" onClick={connectWallet}>
          Connect Wallet
        </button>
      ) : (
